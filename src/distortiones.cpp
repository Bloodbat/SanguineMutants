...
struct Distortiones : SanguineModule {
	...
	Distortiones() {
		...
		configParam(PARAM_LEVEL_1, 0.f, 1.f, 0.65f, "External oscillator amplitude / internal oscillator frequency", "%", 0, 100);
		configParam(PARAM_LEVEL_2, 0.f, 1.f, 0.65f, "Modulator amplitude", "%", 0, 100);
	...
	void process(const ProcessArgs& args) override {
		...
		if (lightsDivider.process()) {
			...
			if (!bModeSwitchEnabled) {
				float zone;
				const uint8_t(*palette)[3] = featureMode != distortiones::FEATURE_MODE_META ?
...
struct DistortionesWidget : SanguineModuleWidget {
	explicit DistortionesWidget(Distortiones* module) {
		...
		addChild(createLightCentered<TinyLight<GreenLight>>(millimetersToPixelsVec(13.849, 58.44), module, Distortiones::LIGHT_MODE));
		...
		addChild(createLightCentered<TinyLight<RedGreenBlueLight>>(millimetersToPixelsVec(14.281, 62.532), module,
			Distortiones::LIGHT_CHANNEL_MODE));
