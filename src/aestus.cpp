...
struct Aestus : Module {
	...
	void process(const ProcessArgs& args) override {
		...
		if (bLightsTurn) {
			...
			lights[LIGHT_MODE + 0].setBrightnessSmooth(mode == 2 ? 1.0 : 0.0, sampleTime);
			lights[LIGHT_MODE + 1].setBrightnessSmooth(mode == 0 ? 1.0 : 0.0, sampleTime);

			lights[LIGHT_RANGE + 0].setBrightnessSmooth(range == 2 ? 1.0 : 0.0, sampleTime);
			lights[LIGHT_RANGE + 1].setBrightnessSmooth(range == 0 ? 1.0 : 0.0, sampleTime);
