		...
		if (!bWantPeacocks) {
			...
			for (int channel = 0; channel < channelCount; ++channel) {
				...
				if (generators[channel].writable_block()) {
					...
					generators[channel].set_pitch(clamp(static_cast<int>(pitch) * 128, -32768, 32767));
				...
				outputs[OUTPUT_HIGH].setVoltage((samples[channel].flags & sanguinetides::FLAG_END_OF_ATTACK) ?
					5.f : 0.f, channel);
				outputs[OUTPUT_LOW].setVoltage((samples[channel].flags & sanguinetides::FLAG_END_OF_RELEASE) ?
					5.f : 0.f, channel);
