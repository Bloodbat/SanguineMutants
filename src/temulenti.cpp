...
struct Temulenti : SanguineModule {
	...
	void process(const ProcessArgs& args) override {
		...
		if (bLightsTurn) {
			...
			lights[LIGHT_MODE + 0].setBrightnessSmooth(mode == bumps::GENERATOR_MODE_AD ? 0.5f : 0.f, sampleTime);
			lights[LIGHT_MODE + 1].setBrightnessSmooth(mode == bumps::GENERATOR_MODE_AR ? 0.5f : 0.f, sampleTime);

			lights[LIGHT_RANGE + 0].setBrightnessSmooth(range == bumps::GENERATOR_RANGE_LOW ? 0.5f : 0.f, sampleTime);
			lights[LIGHT_RANGE + 1].setBrightnessSmooth(range == bumps::GENERATOR_RANGE_HIGH ? 0.5f : 0.f, sampleTime);
